> **SUPERSEDED** by [`../Plan-Master-Roadmap.md`](../Plan-Master-Roadmap.md) (2026-07-02) — the single live roadmap. Retained as milestone history (M0–M3b shipped) and for its Java-reference / what-not-to-port notes.
...
_**M1.6 shipped 2026-06-18** — Elo-on-screen (issue #84). The long-assumed path (an `AccountInfoData` push carrying the new Elo) turned out not to exist: the 2013 server never surfaced post-battle Elo in-client — `BattleFinishedData` and `account/info` (`AccountSvc.java`) carry no rating, and `BattleMonitor.saveRankings()` pushed Elo only to Steam leaderboards, never to the results screen (whose UI is baked into compiled `match_resolution.swf`). Rating is instead surfaced through the **leaderboards page**, now built live from the `ranking` table — real players merged into the preserved `data/lboard.json` baseline, each player shown their own value + rank (new `src/db/leaderboard.ts`; `src/services/game.ts` rewired). The immediate post-battle chat-line idea is split out to #137. 296 tests; manual 2-player verified. See the M1.6 milestone below._
...
- ✅ **Resolved in M1.6 (2026-06-18, #84)** — Elo-on-screen. Investigation found
  the original never surfaced post-battle Elo in-client (no Elo in
  `BattleFinishedData` or `account/info`; `saveRankings()` pushed only to Steam
  leaderboards), so it was surfaced via the DB-driven leaderboards page instead.
  Post-battle chat line split to #137. See the M1.6 milestone.
...
**M1.6 — Elo-on-screen. Shipped 2026-06-18 (#84).**
- ✅ **Investigation outcome (overturned the assumed path):** the 2013 server never
  surfaced post-battle Elo in-client. `BattleFinishedData` (Java + AS3) has no Elo
  field, `account/info` (`AccountSvc.java`) returns no rating, and
  `BattleMonitor.saveRankings()` pushed wins/losses/elo only to **Steam leaderboards**
  (`sendLeaderboardsToSteam`, IDs 131893–131896). The results screen UI is baked into
  the compiled `match_resolution.swf`. So there was no `AccountInfoData` push to port.
- ✅ Surfaced rating through the **leaderboards page** instead: `/game/leaderboards`
  now builds live from the `ranking` table in new `src/db/leaderboard.ts`
  (`buildLeaderboards`), merging real players into the preserved `data/lboard.json`
  historical baseline and giving each requester their own `user_value` / `user_rank`.
  `src/services/game.ts` serves the live build, falling back to the static blob only
  on a DB error. Metric-per-board mapping ported from `tbs.srv.data.LeaderboardData.java`.
- ✅ 296 tests (new `src/db/leaderboard.test.ts` + a `test/routes/game.test.ts` case);
  manual 2-player verified. Closes #84.
- ⏭ **Split out to #137** — an immediate post-battle chat line showing the new rating
  (no client rebuild needed; pending a manual visibility check on the result overlay).
...
- ✅ **Resolved in M1.6 (2026-06-18, #84)** — Elo-on-screen. Surfaced via the
  DB-driven leaderboards page (the original never sent post-battle Elo in-client);
  post-battle chat-line surface split to #137. See the M1.6 milestone.
