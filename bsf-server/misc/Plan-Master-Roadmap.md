...
|---|---|---|---|---|
| **Client auto-retry: never answer a permanent "no" with `404`/`5xx`** (audit R10) | _new_ | SEC | READY — the client re-sends on `0`/`404`/`5xx` with **no attempt cap** on all 23 opted-in txns; `/services/tourney/join` loops today. Spec: [`../docs/client-contract.md`](../docs/client-contract.md) → R10 | S–M |
| Compare the two players' per-turn checksums (audit R15) | _new_ | SEC | READY — we already store both in `Battle.ts:406-409` and never compare; the client's docs claim we do | S |
| Assert the emitted player number fits signed 32 bits (audit R1) | _new_ | SEC | READY — holds in practice, unguarded; a test plus a bound in `accountId.ts` | XS |
| Protocol version `"11"` used as the auth-bypass sentinel (audit R4) | _new_ | MAINT | latent — only bites a client built with a different protocol version | XS |
| Message pushed into an abandoned poll may be dropped (audit R9) | _new_ | MAINT | UNPROVEN — needs a test simulating an aborted request, not more log reading | S |
...
| Retire double-refund race | #144 | POSTPONED | postponed 2026-07-19 — #154 zeroes the retire refund (defuses the renown mint); residual = minor "retired unit can reappear" quirk. **Re-scoped by the client-contract audit (2026-07-28):** the trigger is not only a click race — the client re-sends automatically on `5xx`, and `roster.ts:141-145` updates memory only after the DB write, so a partly-failed write is replayed. #154 still removes the double payment; it does **not** remove the retry loop (see the R10 row below). | S |
