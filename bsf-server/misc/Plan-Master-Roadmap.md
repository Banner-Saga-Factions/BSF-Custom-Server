...
|---|---|---|---|---|
| **Client auto-retry: never answer a permanent "no" with `404`/`5xx`** (audit R10) | #164 | SEC | READY — the client re-sends on `0`/`404`/`5xx` with **no attempt cap** on all 23 opted-in txns; `/services/tourney/join` loops today. Spec: [`../docs/client-contract.md`](../docs/client-contract.md) → R10 | S–M |
| Log why a battle desynced (audit R15) | #165 | MAINT | **Re-scoped 2026-07-30 — not a safety gap.** The clients compare checksums themselves and end the battle on a mismatch; we relay, which is all that is required. We just have no record of *why* a battle ended. A server-side comparison would need new per-turn storage first (`/battle/sync` keeps nothing today) | S–M |
| Assert the emitted player number fits signed 32 bits (audit R1) | #166 | SEC | READY — holds in practice, unguarded; a test plus a bound in `accountId.ts` | XS |
| Protocol version `"11"` used as the auth-bypass sentinel (audit R4) | #167 | MAINT | latent — only bites a client built with a different protocol version | XS |
| Message pushed into an abandoned poll may be dropped (audit R9) | #168 | MAINT | UNPROVEN — needs a test simulating an aborted request, not more log reading | S |
| Discord `account_id` collision — **residual** (shared `ranking` PK + in-battle identity) | #140 | SEC | #146 done + #140 mitigated (PR #156); real fix = server-assigned small `game_id` (crossplay design-B). **Audit R3 adds a player-visible symptom:** matchmaking keys on the derived number (`queue.ts:274,537`), so a colliding player is told they are "already in the queue" and the two can never be matched. That same check is currently the only thing preventing a real desync between them — a server-assigned-id fix must keep an equivalent guard. | deferred |
...
| Retire double-refund race | #144 | POSTPONED | postponed 2026-07-19 — #154 zeroes the retire refund (defuses the renown mint); residual = minor "retired unit can reappear" quirk. **Fold into #164 (2026-07-30).** Review corrected the mechanism: `saveRosterAndAddRenown` is a single synchronous `UPDATE`, so there is no partly-failed write — the hazard is two *overlapping* requests both reading then both adding the refund, which the client's auto-retry can cause. #154 (refund = 0) therefore **fully** removes the double payment, and what remains is the retry loop itself = #164. Optionally close the read-then-write window with a `WHERE`-guarded update (`expandBarracks` has the pattern). | S |
