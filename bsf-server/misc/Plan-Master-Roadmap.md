...
| **A route handler that throws sends no reply at all, and the game waits forever** | #176 | SEC | READY — nothing registers error-handling middleware (`src/app.ts`, `src/index.ts`), and Express 4.18.2 does not catch a rejected promise from an `async` handler. The socket stays open and **no event ever reaches the game**: it does not retry, does not error, and whatever waited on the request stalls. The 5-second timeout that would have rescued it is dead code (`HttpRequest.as:35` — the timer is never started). Wider trigger than #164: any throw on any route. Note a **synchronous** throw behaves differently — Express catches it and answers `500`, which the game *does* retry. | S |
| **A logged-out player is told "forbidden", which the game treats as an ordinary failure** | #180 | SEC | READY — `src/app.ts:106-107` answers `403` for a session key it does not recognise, but `401` is the only code the game reads as *you are logged out*. After a restart the player gets a network-problem banner and is **never returned to login** — visible, but uninterpretable and unrecoverable. | XS–S |
...
| Session key read from the **last** part of the address, so a route with parts after it is refused | #188 | SEC | **Needs an unmatched path to stop answering `404` first** — today's wrong read is what keeps the missing variation route a harmless `403` rather than a `404` the game retries every second. Either #98's route or #164's catch-all clears that; then this is safe. Working precedent: the Steam overlay route, answered at `src/app.ts:85` ahead of the gate. Spec: [`../docs/client-contract.md`](../docs/client-contract.md) → R5 | S |
...
**Phase 1b — two small fixes to what we answer with. This is the front.**
...
1. **#176** — register one error-handling middleware, so a handler that throws answers instead of
   sending nothing. Sending nothing is the worst outcome available, not the mildest: the socket stays
   open, no event ever reaches the game, and it waits forever without retrying or reporting an error.
2. **#180** — answer `401` rather than `403` for a session key we do not recognise, so a player whose
   session died is returned to login. Today they get a network-problem banner and stay stuck behind it.
