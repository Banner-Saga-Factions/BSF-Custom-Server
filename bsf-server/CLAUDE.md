...
5. On match: `tryCreateBattle` recomputes both sides' powers one more time, re-validates the windows (a pair who asked for each other is exempt), and calls `battleHandler.addBattle(parties, mode, perSide, opts)` where `opts` carries `friendly` and the requested map and `perSide` is a two-element `{ power, elo, timer }[]` (earlier-queued entry at `party_index=0`). Each side's `timer` is that player's own request (#213) — the two can differ, and a `0` means no clock, which the game honours by building no countdown at all. The `Battle` constructor pushes `BattleCreateData` to both sessions via `pushData` and removes both from the queue.
