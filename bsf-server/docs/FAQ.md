...
- **All six of the game's run-mode launch options write one setting, and the last one wins** — so a command ending `--developer` stops at the main menu (one click from the town, not a hang), and one ending `--versus_start` reaches the town but silently cancels `--developer`. `--flag=value` and bare `key=value` are never read, and no unrecognised word is ever reported. [`Development.md`](Development.md#which-screen-a-launch-command-lands-on) → *Which screen a launch command lands on*.
