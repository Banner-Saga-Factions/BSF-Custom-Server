...
- **Count players online from `lastPollAt`, never `lastActivity`** — every message the server sends refreshes `lastActivity`, so a crashed game looks alive; the session reaper stays on `lastActivity` on purpose. [`observability.md`](observability.md#why-online-is-not-signed-in) → *Why "online" is not "signed in"*.
