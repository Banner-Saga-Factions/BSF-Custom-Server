...
- **A lookup table keyed by anything from a request must have no prototype** (no built-in entries of its own) — use a `Map` or `Object.create(null)`, never `{}`, which already answers to names such as `constructor` (#311). [`security.md`](security.md#adding-a-route--a-short-checklist) → *Adding a route*.
...
- **A PowerShell check can pass when it should fail** — a script-local `$LASTEXITCODE`, an `$env:` switch left on, or a non-ASCII character read by Windows PowerShell 5.1. [`Development.md`](Development.md#writing-a-powershell-script-that-runs-a-check) → *Writing a PowerShell script that runs a check*.
...
- **`xxd in out` writes the file `out`** — a second filename is an output file, so a command that reads like an inspection truncates it.
- **`git fetch` and `git ls-remote` stop being reads the moment `--upload-pack=` is present**, which runs an arbitrary program — so neither belongs on a permission allowlist with a trailing wildcard.
- **`gh issue view N --comments` prints nothing in Git Bash**, and `GH_PAGER=cat` in front stops the pre-approval matching — read comments with `--json comments --jq '.comments[].body'`. [`../CLAUDE.md`](../CLAUDE.md) → *The backlog, and how work moves*.
