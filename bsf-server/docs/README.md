...
**One half of this is not yet measured, and should not be read as coverage.** The *folder* form of naming paths is proven: `db.md` has used it since July 2026, and every session handed it had opened a file under `src/db/`. `ops.md` also names individual files — the container files and launch scripts at the top of this folder, and the deployment guide inside `docs/` — in forms nothing in this repository has tested and nothing anywhere documents. A session that opens one of those may or may not be handed it. Until somebody opens one in a **fresh** session and reads the record, treat that half as unverified. The session that made the change could not settle it: rules attach when a file is opened, not when a session starts, but nothing in the record shows a rules file created part-way through a session ever attaching in it.
