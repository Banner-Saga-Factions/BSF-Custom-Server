...
**What it costs now — measured 2026-09-20.** The same count, over the 82 sessions on record: this folder's guide was handed to **35** of them, the code traps to **34**, the database rules to **9**, and the operational traps to **8**. The narrowing held — fewer than half of all sessions pay for the guide, and three fifths are never handed the code traps at all. The count also corrected something our own planning documents had assumed: this folder's guide is **not** read at the start of every session; it is read by a little under half. And [`../../REFERENCE.md`](../../REFERENCE.md), which one plan counted as part of the automatic load, is handed over **never** — 0 of 82, by either of the two routes a file can arrive on.

**What that count does and does not cover.** The record it reads is a rolling window of roughly the last thirty days, not the project's whole history, so "82 sessions" means 82 recent ones. It counts whole sessions and not the helper sessions a review spawns, of which there are another 148 on disk — in a project that then ran four-reviewer passes as a matter of routine, that is a real share of the cost this paragraph is trying to put a number on. And it can only see what this repository stores. One thing arriving before any work begins is not in this repository at all: the memory index, loaded for every session in this project. So treat the numbers above as a floor, and keep the repository's own guide short on the strength of the mechanism — every session in the project reads it — rather than on the strength of this count, which cannot see it either.
...
| Read what past work taught us about how we work, when no other guide owns the lesson, and the cases behind the rules in this folder's guide | [`retrospectives.md`](retrospectives.md) |
...
- **What is being worked on, and in what order,** is on the public [BSF Roadmap board](https://github.com/orgs/Banner-Saga-Factions/projects/3). **Live plans** are at [`../misc/`](../misc/); superseded plans are archived outside the public repository.
