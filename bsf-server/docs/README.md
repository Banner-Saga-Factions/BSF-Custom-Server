...
**What it costs now — measured 2026-09-20.** The same count, over the 82 sessions recorded to date: this folder's guide was handed to **38** of them, the code traps to **34**, the database rules to **9**, and the operational traps to **8**. The narrowing held — fewer than half of all sessions pay for the guide, and three fifths are never handed the code traps at all. The count also corrected two things our own planning documents had assumed. This folder's guide is **not** read at the start of every session; it is read by a little under half. And [`../../REFERENCE.md`](../../REFERENCE.md), which one plan counted as part of the automatic load, is handed over **never** — 0 of 82 — because only a `CLAUDE.md` and the rules files are loaded this way, not ordinary documents. The guide that every session really does pay for is the repository's own, one level up; it is the one worth keeping short, and the same change that added this paragraph moved about 3,000 bytes of reference-lookup material out of it and into the file already meant to hold it.
