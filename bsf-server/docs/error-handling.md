...
| `503` | **Not emitted anywhere today.** Held in reserve: a `503` saying the server is down for maintenance is the only refusal the game will not re-send | **JSON** whose body contains `Offline for Maintenance` or `game_rebooting` | abandons that request and shows the player a maintenance dialog |
...
- **`404` — missing resource.** `app.ts` -> the debug/renown route (no such session); `download.ts:19,24` (factions size/checksum unset); `roster.ts` -> the unit and template lookups in promote / rename / retire / hire / stats-purchase / stats-reset; `Battle.ts` -> the battle middleware (no battle with that id). The second one here, for a turn the opponent had not taken yet, is gone: since #213 `/battle/query` answers an empty `200` instead, because the client re-sent the `404` for ever.
