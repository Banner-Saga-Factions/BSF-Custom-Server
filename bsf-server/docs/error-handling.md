...
- **`404` — missing resource.** `app.ts` -> the debug/renown route (no such session); `download.ts:19,24` (factions size/checksum unset; neither route can be reached, #308); `roster.ts` -> the unit and template lookups in promote / rename / retire / hire / stats-purchase / stats-reset. `Battle.ts` raises none any more, because the client re-sent both of its `404`s for ever: since #213 `/battle/query` answers an empty `200` for a turn the opponent has not taken yet, and since #164 the battle middleware answers an empty `200` for a battle we no longer hold (`400` for the turn query).
