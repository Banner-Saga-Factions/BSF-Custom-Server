...
- **`429` — too many requests.** `game.ts:35` (a second concurrent poll while `pollingActive`); `auth.ts` -> the login limiter (>5/min/IP — JSON `{error:"Too many login attempts…"}`; skipped under `NODE_ENV=test`; counts each player rather than everybody together only because `app.ts` -> the trust-proxy setting tells the server one proxy of ours is in front, #284).
