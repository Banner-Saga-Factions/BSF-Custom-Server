...
|---|---|---|---|
| `200` | OK — inline data, or a fire-and-forget ack (the real reply arrives by long-poll). Also any battle request for a battle we no longer hold (#164) | empty or JSON | `noticeOk()` → flows to the response callback |
| `400` | Bad input (validation failed), or an address no route answers (#164) | mostly **bare**, some **JSON** `{error,…}` | **treated "OK"** (it's `<401`) → flows to the callback, *not* the degraded-connection UI |
...
- **`404` — missing resource.** `app.ts` -> the debug/renown route (no such session); `download.ts:19,24` (factions size/checksum unset; neither route can be reached, #308); `roster.ts` -> the unit and template lookups in promote / rename / retire / hire / stats-purchase / stats-reset. `Battle.ts` raises none any more, because the client re-sent both of its `404`s for ever: since #213 `/battle/query` answers an empty `200` for a turn the opponent has not taken yet, and since #164 the battle middleware answers an empty `200` for a battle we no longer hold.
