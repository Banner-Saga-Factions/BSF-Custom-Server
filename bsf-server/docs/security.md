...
| **Login rate-limit** | **5 / 60 s / IP** on the Steam sign-in route, in-memory; returns `429` with a JSON message; skipped under `NODE_ENV=test`. It counts each **player** only because the server is told one proxy of ours sits in front of it — without that, every request carries Caddy's address and one cap is shared by everybody (#284). The Discord sign-in route has no cap at all; see *What is NOT protected today* | `auth.ts -> the login limiter` (#56); `app.ts -> the trust-proxy setting` (#284) |
...
| **`/debug/*` gating** | The debug router mounts only when `NODE_ENV !== "production"`, with a loud boot warning when it's on | `app.ts -> the debug-router block`, `index.ts -> the debug-routes boot warning` |
...
- **The login rate-limit is per-process and in-memory** (`auth.ts -> the login limiter`). It does not survive a restart and does not hold across multiple hosts — a scale-out would need a shared store (e.g. Redis).
- **The sign-in cap counts each player only while exactly one proxy of ours is in front.** `TRUST_PROXY` (#284) tells the server to use the address Caddy forwards rather than Caddy's own. It says *one* hop, so putting a content delivery network in front of Caddy would quietly undo it: Caddy would then record the delivery network's edge, and everybody behind one edge would share a cap again — with the startup line still reporting the setting as on. **The startup line proves the setting, not the result.** There is no such service today; this is recorded so a future one does not re-open #284 silently. Running the image directly with a published port and nothing in front must leave the setting off, or the forwarded address becomes a header any player can write.
- **The Discord sign-in route has no rate limit at all** (#301). `/login/discord/*` is mounted outside `/services`, so it also skips the session gate, and the route that begins the sign-in mints a fresh single-use token into an in-memory map on every anonymous request, with a five-minute life and no cap on how many may exist at once. Nothing slows a flood, and the map grows while it lasts. The cap in the table above covers the Steam route only.
- **No in-process TLS.** The server listens with plain `http` on `:8082` (`index.ts`); transport encryption is assumed to be terminated upstream by Caddy. Running it directly exposed would be cleartext.
...
- **Discord id collision (#140 — not labeled security).** Two Discord Snowflakes that share their low 30 bits map to the same 32-bit in-game `account_id` (`discord.ts:175`). They stay distinct DB rows (keyed on the full id string), but would share an in-game identity. Integrity-adjacent; listed here as a known limitation.
...
3. **Validate and clamp input.** Treat every field as hostile — check types, lengths, and ranges (the roster routes are the model). When a value from the request is used to look something up, keep the table in a `Map` or an object made with `Object.create(null)`, never a plain `{}`: a plain object already carries about a dozen built-in names, such as `constructor`, so looking one of them up finds an entry nobody stored. Before #311 that let a request carrying such a name instead of a sign-in key pass for a signed-in player, and the same slip silently dropped a row from a test's own table of results during that fix's review, so it applies to test code too.
