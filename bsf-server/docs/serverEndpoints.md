...
---
  ...
  **Long-poll behaviour:** if `session.data` already has buffered messages, the response returns them immediately and clears the buffer. Otherwise the request waits up to **5 seconds** for a `'data'` event from `session.pushData()`. On timeout the response is `200 OK` with an **empty JSON array**. The client then re-issues the poll **immediately** — it does not pause between polls and does not back off after an error, so the next request can arrive the instant this one is answered. A `pollingActive` flag prevents two concurrent polls per session — the second returns `429`; measured at about 7% of polls during a live battle, which is the normal double-poll overlap rather than a stuck session. The 5-second hold is correct as it stands: a captured battle showed 85% of polls waiting the full five seconds, which proves the client does not abandon the request early. See [`client-contract.md`](./client-contract.md) → R7–R9.
