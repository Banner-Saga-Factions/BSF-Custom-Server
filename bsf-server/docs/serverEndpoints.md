...
## Account Endpoints
  ...
  **Side effects:** updates `session.accountData.roster_json` in memory and persists via `saveRoster()` to SQLite. `roster_rows` is **only** mutated by `POST /roster/unlock` (calling `expandBarracks()`), which costs 60 renown per row and rejects with `400 {"error":"barracks at max"}` once `roster_rows >= MAX_ROSTER_ROWS` (8).
  ...
  `deltas`|`number[]`|Per-stat deltas, parallel to `stats`. Must be integers in `[-20, 20]`. **Negative values are legitimate, not an error** — the stat panel *subtracts* a point on right-click, so moving points back out of a stat sends a negative delta. `0` is tolerated as a no-op (the client adds any interacted stat to its batch, so `+1 then -1` yields `0` for that stat).
...
---
  ...
  **Long-poll behaviour:** if `session.data` already has buffered messages, the response returns them immediately and clears the buffer. Otherwise the request waits up to **5 seconds** for a `'data'` event from `session.pushData()`. On timeout the response is `200 OK` with an **empty JSON array**. The client then **waits a fixed gap before polling again — 3 seconds normally, 1 second during a battle** (`HttpCommunicator._pollTimeMs`, passed as the pre-send delay of `HttpAction.send`). The gap never grows; there is no back-off after an error. So worst-case delivery for a pushed message is that gap, not the 5-second hold — a message pushed into an already-open poll goes out immediately. A `pollingActive` flag prevents two concurrent polls per session — the second returns `429`; measured at about 7% of polls during a live battle, which is the normal overlap around a gap change (battle start drops it 3 s → 1 s) rather than a stuck session. The 5-second hold is correct as it stands: a captured battle showed 85% of polls waiting the full five seconds. See [`client-contract.md`](./client-contract.md) → R7–R9.
