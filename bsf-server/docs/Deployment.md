...
- **`[BOOT] NODE_ENV=production trust_proxy=1`, and *no* line reading "debug routes are ENABLED".** `trust_proxy=1` is the part that says the server is using the address Caddy forwards, so the cap of five sign-ins a minute counts each player rather than counting everybody as Caddy (#284). `trust_proxy=false` behind Caddy means every player shares one cap of five sign-ins a minute.
- **No `[BOOT]` line at all means the app never started**, not that the log is quiet. It refuses to start when the signing key is missing, or when the proxy setting has a value it does not recognise, and that refusal happens before the `[BOOT]` line is printed. Read `docker compose logs app` without the filter — the reason is the line beginning `Error:`, above the stack trace and the closing `Node.js` version banner.
