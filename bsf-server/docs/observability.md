...
|---|---|---|---|
| `[BOOT]` | `src/index.ts` | `[BOOT] NODE_ENV=production trust_proxy=1` | Process started; which environment, and whether it believes the address a proxy of ours forwards. `trust_proxy=1` means the cap of five sign-ins a minute counts each player separately; `false` means it counts whoever connected, which behind a proxy is the proxy — so every player shares one cap (#284). **No `[BOOT]` line at all** means the process never started: it refuses to when a required setting is missing or misspelled, and that refusal happens before this line is printed. |
