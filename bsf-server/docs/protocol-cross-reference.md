...
| `POST /services/roster/unit/hire/:session_key?` | `tbs/srv/web/svc/roster/unit/hire/UnitHireSvc.java` | shipped — sends no renown balance afterwards (diverges from Java, which did; see [`RenownMessage`](./dataStructures.md#renownmessage)) |
...
| `POST /services/roster/unit/retire/:session_key?` | `tbs/srv/web/svc/roster/unit/retire/UnitRetireSvc.java` | shipped — refunds the renown spent promoting the unit, never its hire price (#95); diverges from Java, which refunded nothing |
