...
const tryCreateBattle = (a: QueueItem, b: QueueItem): boolean => {
    ...
    // counted. Not awaited, and it never fails -- see src/services/activityStats.ts.
...
export const expireStaleSearches = (now: number = Date.now()): void => {
    for (let i = gameQueue.length - 1; i >= 0; i--) {
        ...
        if (now - item.queuedAt.getTime() > QUEUE_TIMEOUT_MS) {
            ...
            // #267. Not awaited, and it never fails -- see src/services/activityStats.ts.
...
QueueRouter.post("/start/:session_key", async (req, res) => {
    ...
    // awaited, and it never fails -- see src/services/activityStats.ts.
