...
### Quick matches no longer hang when a slightly-progressed player can only find brand-new opponents

Until now, a player whose party had been promoted a little — say one rank-3 hero plus two rank-2 units, totalling power 4 — could sit in the quick-match queue indefinitely when the only other players around were brand-new accounts at power 0. The matchmaker advertised that it would relax its pairing tolerance over the first 90 seconds of waiting, but the relaxation it was willing to grant *depended on each player's own party strength*. Brand-new players were given a narrower window than progressed players, and a pairing only happened when *both* sides were willing to accept the gap. So the brand-new player's window always blocked the 4-point gap, no matter how long either of them waited.

Why it mattered: on a small player base where most active accounts are still on their starter party, this looked like the matchmaker was simply broken. A player who'd promoted a single unit could queue up, wait several minutes, and see no signal as to why nothing was happening. The original 2013 Stoic server had this same asymmetric cap by design — it was meant to protect very-new players from getting stomped by veterans — but on this server, with so few active accounts, it just blocks legitimate matches without giving the new player anything in return.

The fix removes the per-player asymmetry entirely. Every player's pairing tolerance now grows to the same global maximum (4 power levels) over the same 90-second window, regardless of their current party strength. A power-0 player still won't pair *instantly* with a power-4 player — the window still has to widen over the 90-second ramp — but they will pair eventually. Ranked and tourney matches are unaffected because both modes require exact party strength on both sides.

A two-client manual test confirmed the new behaviour: two players at party power 4 with a 133-point Elo gap paired after about one minute of waiting, matching the expected ramp time. The original "Quick match available at power N" banner shows the correct number, and the resulting battle proceeds normally.

Two diagnostic log lines were added during the investigation and left in place for ongoing operability. The first prints a per-unit RANK breakdown every time a player joins the matchmaking queue (prefix `[QUEUE]`); the second prints the full roster RANKs every time a client requests its account info from the server (prefix `[ACCOUNT_INFO]`). Together they let an operator answer "why is my party showing as power N?" in one glance at the log instead of by chasing the math through the database.

Side note that surfaced during the same investigation: the AS3 game client's `AccountInfoTxn` silently swallows any parse error from the `/account/info` response, falling back to whatever roster it already had cached in `global_0.sol`. That can make the client render stale ranks even when the server is correct. The most common trigger is an `entityClass` value in `data/acc.json`'s `purchasable_units` that the client doesn't recognise (the client parses its valid class list from `character_classes.json.z`). That class of bug was a contributing cause of the apparent "wrong power on the banner" mystery and is fixed separately by commit `8c81f03`, which removed the unrecognised `wardog` and `spearman` entries from `purchasable_units` until issue #101 ships the class-registry plumbing.

*Technical:* `src/services/queue.ts` — deleted `computeDynamicPowerMax` (formerly `eq = VS_WINDOW_EQ_CONST + trunc((power + VS_WINDOW_EQ_DELTA) / VS_WINDOW_EQ_DENOMINATOR)` clamped to `[VS_WINDOW_POWER_MIN, VS_WINDOW_POWER_MAX]`, returning 3 for powers 0–3 and 4 for powers 4+) along with the three `VS_WINDOW_EQ_*` constants that fed it. `createQueueItem` now assigns `threshold_power_max: VS_WINDOW_POWER_MAX` directly; the redundant reassignments in `tryCreateBattle` and `processMatches` were removed. Deliberate divergence from `tbs.srv.worker.VsWorker.java:246-254` — that piece of the M2 port is no longer faithful and is called out as such; a future re-port should not "fix" it. `src/services/matchmaker.test.ts` — removed the `computeDynamicPowerMax` describe block and its import. `src/services/queue.test.ts` and `src/services/matchmakerTick.test.ts` — updated test fixtures from `threshold_power_max: 3` to `4` to match what `createQueueItem` now produces (the existing assertions still pass since they match same-power pairs). New `[QUEUE]` log block added to `/start` POST handler in `queue.ts` after `calculateLevel(...)`; new `[ACCOUNT_INFO]` log block added to `/info` GET handler in `src/services/account.ts` after the null-check on `session.accountData`. Both reuse the existing `buildOrderedPartyDefs` helper and read `RANK` via `stats.find(s => s.stat === "RANK")?.value ?? 1`. `bsf-server/CLAUDE.md` matchmaking-lifecycle bullet refreshed to drop the `computeDynamicPowerMax` reference. The `BSF_MATCHMAKER_LEGACY=true` rollback flag still works — `legacyMatchmaking` was unaffected by this change.
