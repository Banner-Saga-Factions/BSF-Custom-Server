...
When the game hires a unit it names it itself, such as "archer_0", and keeps using that name. The
server stored the unit under a name of its own, so for the rest of that session retiring, promoting or
training it was answered "not found", which the game repeats every two seconds without end, and putting
...
anything it cannot do with an answer the game does not repeat. A repeated promotion, stat purchase or
new barracks row is still applied again, as the original server did.

*Technical:* `src/services/roster.ts` — hire stores `new_unit_id` as sent (the `_start_` renaming is gone), requires it to be text matching `^[A-Za-z0-9_]{1,64}$`, answers `400` for an unknown template, and treats an id clash as a repeat (`200`, no charge, no push) only when the new `Session.recentHires` map (`src/services/auth/auth.ts`) holds that id from the last 60 s and the class matches, checked before renown and space; retire answers `200` with one log line for a unit not in the roster; promote and stat purchase answer `400` for an unknown unit. `src/services/battle/Battle.ts` — `killReports`, `unitKillCounts` and `killReportKillers` are made with `Object.create(null)` at both levels (#311 rule), since a hire id may now be a built-in word. Tests in `test/routes/roster.test.ts` and `src/services/battle/Battle.test.ts`. Closes #164 and #304.
...
Nearly every game request ends in the player's sign-in key, and the server looks that key up in its list
of signed-in players. That list came with a few built-in names of its own, so a request using one of those
...
built-in names. Such a key is refused, and such a battle id is answered like any battle the server
does not hold.
...
page. But a second program takes the traffic first and passes it on, and the server had
never been told that, so every request looked as though it came from that one program --
and the five a minute were shared by everybody at once: five sign-ins a minute for the
whole server rather than for each person. The sixth is turned away, and the game does
not try again by itself. The server is now told that exactly one program stands in front
of it, and only where one really does.

*Technical:* adds `trustProxy()` to `src/const.ts` and a conditional `app.set("trust proxy", 1)` in `src/app.ts`, called after `config()` so it is read once dotenv has run. Express 4.18.2 compiles a numeric hop count to `(a, i) => i < val`, which `proxy-addr` 2.0.7 then walks, so `req.ip` becomes the **rightmost** `X-Forwarded-For` entry -- the one the immediate peer wrote -- and anything further left is never reached; `true` would trust the whole chain and return the leftmost, which `express-rate-limit` 8.5.1 only logs once per process as `ERR_ERL_PERMISSIVE_TRUST_PROXY` rather than refusing. `TRUST_PROXY` is set in `docker-compose.yml`'s `app.environment` block rather than `.env`, because a Compose `environment:` entry overrides `env_file:` and that file is what puts Caddy in front; unset means off, and an unrecognised value throws at `src/app.ts` import time, so `vitest.config.ts` pins it empty beside `STARTING_RENOWN`/`SKIP_TUTORIAL` -- without that pin a stray export would fail every `test/routes` file. `src/index.ts`'s `[BOOT]` line now prints `trust_proxy=${app.get("trust proxy")}`, read back out of Express rather than re-parsed. New `test/routes/login-rate-limit.test.ts` rebuilds the app under `NODE_ENV=development` (the limiter's `skip` disables it under `test`) and sends empty bodies, since the limiter counts a request before the handler answers `400`. Replaces the stale `auth.ts:207-214` / `auth.ts:203-205` anchors in `docs/security.md` and `docs/error-handling.md` with prose, per `error-handling.md`'s own rule, and records two boundaries in `docs/security.md`: a delivery network placed in front of Caddy would silently re-open this, and `/login/discord/*` has no cap at all (filed as #301).
### The guide every session reads is a fifth smaller, and now has a limit

Some guides reach a session before it does any work, so every word is paid for, needed or not. The
one every session reads was largely lookup tables — which copy of the original game to read, and
which twelve of its files are out of date. They have moved to the reference page meant for them,
which nothing reads automatically. Checking them settled an old doubt: the fifty files carrying the
shape of every message between game and server had no saved comparison report, and re-running it
shows them unchanged.

A new build check now fails any pull request pushing one of those guides past a set size. The last
one cut in half grew back by 59% in eleven days, and nothing noticed.
