...
.DESCRIPTION
    ...
    If both pass, it commits with SKIP_SIMPLE_GIT_HOOKS=1 set for that one git invocation: the
    hook's own documented off switch, not `git commit --no-verify` — it skips a repeat of a check
    this script just ran, not the check itself. If either step fails, nothing is committed and
    whatever was already staged stays staged.
